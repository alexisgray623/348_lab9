...
void printMothlySales(void)
{
    ...
    while (fgets(line, sizeof(line), file))         // Reads file line by line
        {printf("%-15s %s", Months[i++], line);}    // Prints the month followed by the $$ that month
...
void printSalesSummary(void)
{
    ...
    {
        temp = strtod(line, &ptr);          // turns the numbers from strings to floats
        avg += temp;                        
        if (max < temp)
        {
            max = temp;
